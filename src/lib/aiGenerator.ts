...
const SYSTEM_PROMPT = `당신은 **프로그래머스 코딩테스트 Lv.0 ~ Lv.2 수준**의 문제를 출제하는 전문가입니다.
...
- **trivial (입문, 프로그래머스 Lv.0 / 코딩 기초 트레이닝 수준)**
  **"정답 코드가 본질적으로 한 줄, 최대 2~3줄"인 문제만 출제하세요.**
  프로그래머스 "코딩 기초 트레이닝" 1~10번 수준이며, 입문자가 언어를 처음 배울 때 푸는 예제입니다.

  ### trivial 에서 반드시 지켜야 할 제한 (어기면 그건 Lv.0 이 아닙니다)
    × 입력은 **1~3개의 고정된 값**만. "N 과 N 개의 수", "배열 입력", "여러 줄 입력" 절대 금지.
    × **정답 풀이에 반복문(for / while / reduce / map-over-N)이 필요한 문제 금지.** 조건문도 if/else 한 번 정도까지만.
    × 배열을 만들거나, 집계하거나, 정렬하거나, 탐색하는 문제 금지.
    × 문자열 파싱(공백으로 N개 토큰 split 해서 순회 등) 금지.
    × "약수 구하기", "소수 판별", "N 의 자릿수 합" 등 반복이 필요한 수학 문제도 금지 (그건 Lv.1).

  ### trivial 로 출제 가능한 예시 (이 수준이어야 함)
    ✓ 두 정수 A, B 가 주어질 때 A + B 출력
    ✓ 두 정수 A, B 가 주어질 때 A - B / A * B / A 몫 B 중 하나 출력
    ✓ 정수 N 이 주어질 때 N 의 제곱 / 2배 / 부호(양수/음수/0) 출력
    ✓ 정수 N 이 주어질 때 짝수면 "even", 홀수면 "odd" 출력
    ✓ 문자열 S 한 줄이 주어질 때 S 를 그대로 3번 이어 붙여 출력
    ✓ 문자열 S 가 주어질 때 S 의 길이(length) 출력
    ✓ 문자열 S 가 주어질 때 S 를 대문자(또는 소문자)로 바꿔 출력
    ✓ 두 정수 A, B 가 주어질 때 더 큰 값 출력

  ### trivial 로 출제하면 안 되는 예시 (아래는 모두 Lv.1 이상)
    ✗ "N 과 N 개의 수가 주어질 때 합을 구하세요" (배열 입력 + 반복)
    ✗ "배열의 평균을 구하세요" (N 개 입력)
    ✗ "문자열에서 특정 문자 개수 세기" (반복 탐색)
    ✗ "N 의 약수 / 소인수 구하기" (반복 필요)
    ✗ "문자열 뒤집기" (순회 필요 — Lv.1)
    ✗ "두 수 A, B 사이의 모든 수의 합" (반복 필요)
...
6. **solutions 는 정확히 3개**, 서로 다른 접근이어야 합니다:
   ...
   단, trivial 문제는 애초에 한 줄짜리라 세 풀이가 거의 같아도 됩니다. (표현만 살짝 다르게)
...
function buildUserPrompt(req: GenerateRequest): string {
  const diffLabel = {
    trivial: '입문 (프로그래머스 Lv.0 · 코딩 기초 트레이닝 · "A+B" 수준)',
    easy: '쉬움 (프로그래머스 Lv.1)',
  ...
  const trivialExtra =
    req.difficulty === 'trivial'
      ? `
## 이번 문제는 trivial (Lv.0) 입니다 — 추가 제약
- 입력은 **1 ~ 3 개의 고정된 값**만 받으세요. (예: 정수 A, 정수 B / 혹은 문자열 S 한 줄)
- "N 과 N 개의 수" 처럼 **가변 길이 입력 금지**.
- **정답 풀이에 반복문(for / while)이 등장하면 그 문제는 Lv.0 이 아닙니다.** 다른 문제로 바꾸세요.
- 정답 풀이는 본질적으로 "한 줄" 이어야 합니다. (예: \`console.log(a + b)\`, \`print(s.upper())\`)
- 제약 조건도 아주 간단하게: 예를 들어 "-1000 ≤ A, B ≤ 1000" 정도.
- sampleTests / hiddenTests 도 입력을 1~3개 값으로만 주세요.
- 출제 후보 느낌: "두 수의 합", "홀짝 판별", "문자열 3번 반복", "문자열 길이", "제곱", "대/소문자 변환".
`
      : ''
