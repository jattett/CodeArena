...
| Java | **로컬 `javac` / `java`** (백엔드 child_process) · Piston 폴백 | JDK 설치 권장 |
| C# | **로컬 `dotnet`** (백엔드 child_process) · Piston 폴백 | .NET SDK 설치 권장 |

Java/C# 는 우선 **로컬 백엔드가 시스템에 설치된 툴체인**으로 실행합니다. 설치되지 않았다면 자동으로 Piston 인스턴스로 폴백합니다.

**macOS 설치 예시**

```bash
brew install openjdk            # Java (javac / java)
brew install --cask dotnet-sdk  # C# (dotnet)
```

설치 후 터미널을 재시작하면 `⚙️ 설정` 모달의 "Java / C# 실행 환경" 섹션에 ✓ 표시로 상태가 자동 반영됩니다.

> **Piston 폴백 (선택)**: 로컬에 JDK/.NET 을 설치할 수 없는 환경에서만 필요합니다.
> emkc.org 공개 API 는 2026-02-15 부터 whitelist 전용으로 바뀌었으므로 자체 호스팅을 권장합니다:
>
> ```bash
...
> ```
>
> 웹 앱 **⚙️ 설정 → Piston URL** 에 `http://localhost:2000/api/v2/piston/execute` 입력.

### 요청 흐름

- 로컬 툴체인 사용 시: `브라우저 → /api/run/local → child_process (javac/java 또는 dotnet)`
- Piston 사용 시: `브라우저 → /api/run/piston → 지정된 Piston 인스턴스`
...
- Java / C# 실행은 Piston 인스턴스에 의존합니다. 2026-02-15 부터 emkc.org 공개 API 는 whitelist 전용이므로 **자체 호스팅**이 권장됩니다 (위 "실행 방식" 참고).
