...
| Java | **Piston** (로컬 백엔드 프록시) | 자체 호스팅 권장 |
| C# | **Piston** (로컬 백엔드 프록시) | 자체 호스팅 권장 |

> ⚠️ **2026-02-15 업데이트**: emkc.org 공개 Piston API 는 whitelist 전용으로 바뀌어 인증되지 않은 요청은 에러를 받습니다. 아래 중 하나를 사용하세요.
>
> **1) 자체 Piston 호스팅 (권장)**
>
> ```bash
...
> ```
>
> 실행 후 웹 앱의 **⚙️ 설정 → Piston URL** 에
> `http://localhost:2000/api/v2/piston/execute` 를 입력하세요.
>
> **2) 서버 기본값 지정**
>
> `PISTON_URL=http://your-instance:2000/api/v2/piston/execute npm run dev`

Java/C# 요청은 프론트에서 직접 외부로 나가지 않고 로컬 백엔드의 `/api/run/piston` 엔드포인트를 거쳐 지정된 Piston 인스턴스로 중계됩니다 (CORS · 에러 처리 편의 목적).
